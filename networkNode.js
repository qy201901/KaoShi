...
app.get('/test', function (req, res) {
   //res.send("hello");
    ...
    {
		var bc = blockchain.chain[i];
		if (isNaN(parseInt(bc.carPlate)))
		{
			continue;
		}		
		sum = sum + parseInt(bc.carPlate);
		n++;
    }
	avg = sum/n;
	avg = avg.toFixed(2);
	res.render('test', { title: 'test', user: user, users: users, blockchain:blockchain ,sum:sum ,avg:avg});
});
...
function isValidSignature(_d) {
    // TODO: check if req signature matches RSA key(the private key is 397, the public key 43333) 
    public_key=calcDecrypt(257, 281, _d);
    if (public_key !== 43333 ){
        return false;   
    }
    else {
...
function isValidMeta(body) {
    return (isValidCarPlate(body.carPlate) && isValidSignature(body.Private_key));
}
